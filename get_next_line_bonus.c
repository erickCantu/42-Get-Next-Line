...
/*   Updated: 2026/01/15 12:53:09 by ecantu-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_gnl_lstdelone(t_str_prt *lst)
{
	free(lst->buff_rd);
	lst->buff_rd = NULL;
	free(lst->ovf);
...
t_str_prt	*ft_gnl_ovf_check(t_str_prt **ls_gnl)
{
	(*ls_gnl)->str[0] = 0;
	(*ls_gnl)->buff_rd[0] = 0;
	...
	(*ls_gnl)->ovf[0] = 0;
	(*ls_gnl)->len_rd = read((*ls_gnl)->fd, (*ls_gnl)->buff_rd, BUFFER_SIZE);
	if ((*ls_gnl)->len_rd < 0 || (((*ls_gnl)->len_rd == 0)
			&& (*ls_gnl)->str[0] == 0))
	{
		ft_gnl_lstdelone((*ls_gnl));
		(*ls_gnl) = NULL;
		return (NULL);
	}
...
{
	char				*str_rsp;
	static t_str_prt	*arr_fd[4096];

	if (fd < 0)
		return (NULL);
	if (!arr_fd[fd])
	{
		arr_fd[fd] = ft_gnl_lstnew(&fd, '\n');
		if (!arr_fd[fd])
			return (NULL);
	}
	ft_gnl_ovf_check(&arr_fd[fd]);
	if (!arr_fd[fd])
		return (NULL);
	str_rsp = ft_strdup(arr_fd[fd]->str);
	return (str_rsp);
}
/*
int	main(void)
